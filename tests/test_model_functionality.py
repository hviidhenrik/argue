...
def df_predict_data():
    return pd.DataFrame(
        {"x1": [0, 1, 2, -1, 4, 100, -100, 8.22], "x2": [0, 1, 2, -1, 4, 100, -100, 2]}
    )
...
def test_argue_lite_fit_and_predict(df_fit_data, df_predict_data):
    ...
    expected_preds = np.array(
        [0.4818, 0.4375, 0.3942, 0.4726, 0.3652, 0.3171, 0.3735, 0.2927],
        dtype="float32",
    )
    ...
    assert type(final_preds) == np.ndarray
    assert len(final_preds) == 8
    assert np.sum(final_preds.reshape((8)) - expected_preds) == 0
...
def test_baseline_autoencoder_fit_and_predict(df_fit_data, df_predict_data):
    ...
    model = BaselineAutoencoder(
        input_dim=2, latent_dim=1, test_set_quantile_for_threshold=0.995
    )
    model.build_model(
        encoder_hidden_layers=[6, 5, 4, 3, 2],
        decoders_hidden_layers=[2, 3, 4, 5, 6],
