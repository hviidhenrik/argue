...
if __name__ == "__main__":
    ...
    df_normal_A.to_csv(path / "data_covtype_normal_A_large.csv", index=False)
    df_anomaly_A.to_csv(path / "data_covtype_anomaly_A_large.csv", index=False)
    df_normal_B.to_csv(path / "data_covtype_normal_B_large.csv", index=False)
    df_anomaly_B.to_csv(path / "data_covtype_anomaly_B_large.csv", index=False)

    make_and_save_debugging_dataset(df_normal_A, size=30000,
                                    filename=path / "data_covtype_normal_A_small.csv")
    make_and_save_debugging_dataset(df_normal_A, size=3000,
