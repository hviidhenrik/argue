...
class AdversarialAutoencoder:
    ...
    def _generator_loss(fake_output, loss_weight: float = 1):
        cross_entropy = tf.keras.losses.BinaryCrossentropy(from_logits=True)  # TODO should this be MSE?
...
if __name__ == "__main__":
    ...
    aae.fit(df_features_scaled[:10000,], dropout_fraction=0.3, epochs=10, batch_size=256, verbose=2)
    ...
    foo = aae.predict(df_features_scaled[10000:10200])
    foo = pd.DataFrame([foo[0,:], df_features_scaled[0,:]])
