...
if __name__ == "__main__":
    ...
    if USE_SAVED_MODEL:
        ...
        model = ARGUE(input_dim=len(df_train.columns[:-1]),
                      ...
                      latent_dim=5)
        model.build_model(encoder_hidden_layers=[25, 20, 15, 10],
                          decoders_hidden_layers=[10, 15, 20, 25],
                          alarm_hidden_layers=[60, 50, 40, 30, 20, 10],
                          gating_hidden_layers=[60, 50, 40, 30, 20, 10],
                          all_activations="tanh")
        model.fit(df_train.drop(columns=["class"]), df_train["class"], epochs=20, number_of_batches=8, batch_size=256,
    ...
    print("Alarm probabilities:\n ", model.predict_alarm_probabilities(df_test))
    print("\nGating weights:\n ", model.predict_gating_weights(df_test))
    print(f"\nFinal anomaly probabilities:\n {np.round(model.predict(df_test), 4)}")
    model.predict_plot_anomalies(df_test, df_test.index, moving_average_window=40)
    # df_preds = pd.DataFrame(model.predict(df_test), index=df_test.index, columns=["Anomaly probability"])
    # df_preds["Average"] = df_preds.rolling(window=40).mean()
    # df_preds.plot(subplots=True)
    plt.show()
