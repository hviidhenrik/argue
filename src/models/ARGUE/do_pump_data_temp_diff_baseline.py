...
from src.models.ARGUE.baseline_autoencoder import BaselineAutoencoder
...
if __name__ == "__main__":
    ...
    for q in quantiles:
        ...
        if USE_SAVED_MODEL:
            ...
            plt.savefig(get_ARGUE_path() / "plots" / f"Baseline_pump30_sanitycheck_reconstructions_q-{q}.png")
            # plt.show()

            model.predict_plot_reconstructions(df_test)
            plt.suptitle(f"Baseline Autoencoder Test set, test quantile = {q}")
            plt.savefig(get_ARGUE_path() / "plots" / f"Baseline_pump30_test_reconstructions_q-{q}.png")
            # plt.show()

            windows_hours = list(np.multiply([8, 24], 40))
            model.predict_plot_anomalies(df_train_sanity_check, window_length=windows_hours)
            plt.suptitle(f"Baseline Autoencoder Sanity check, test quantile = {q}")
            plt.savefig(get_ARGUE_path() / "plots" / f"Baseline_pump30_sanitycheck_preds_q-{q}.png")
            # plt.show()

            # predict the test set
            model.predict_plot_anomalies(df_test, window_length=windows_hours)
            plt.suptitle(f"Baseline Autoencoder Test set, test quantile = {q}")
            plt.savefig(get_ARGUE_path() / "plots" / f"Baseline_pump30_testset_preds_q-{q}.png")
            # plt.show()
