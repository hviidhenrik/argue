...
class ARGUELiteSim:
    ...
    def _init_optimizer_wrapper(
        ...
        if decay_after_epochs is None:
            optimizer = self._init_optimizer(
                optimizer=optimizer_object, learning_rate=learning_rate
            )
        else:
            optimizer = self._init_optimizer_with_lr_schedule(
    ...
    def build_model(
        ...
        self.full_model = Model(
            inputs=self.input_to_decoders.input,
            outputs=[self.input_to_decoders.output, self.input_to_alarm.output],
        )
        self.full_model.trainable = True
        self.full_model.output_names[0] = "AE"
        ...
        if make_model_visualiations:
            ...
            tf.keras.utils.plot_model(
                self.full_model,
                to_file=figures_path / "full_model.png",
                show_shapes=True,
                show_layer_names=True,
            )
    ...
    def fit(
        ...
        optimizer = self._init_optimizer_wrapper(
            decay_after_epochs,
            batch_size,
            epochs,
            learning_rate,
        ...
        self.full_model.compile(
            optimizer=optimizer, loss="binary_crossentropy", metrics=["MAE"]
        )
        self.full_model.fit(
            x=x_train.drop(columns=["partition"]),
    ...
    def save(self, path: Union[Path, str] = None, model_name: str = None):
        ...
        vprint(self.verbose, "\nSaving model...\n")
        model_name = model_name if model_name else "argue_lite_simultaneous"
