...
from src.models.argue_lite_simultaneous import ARGUELiteSim
...
if __name__ == "__main__":
    ...
    plt.savefig(get_figures_path() / "ARGUELiteSim_pump30_sanitycheck_reconstructions.png")
    # plt.show()

    model.predict_plot_reconstructions(df_test)
    plt.suptitle("ARGUE LITE SIM Test set")
    plt.savefig(get_figures_path() / "ARGUELiteSim_pump30_test_reconstructions.png")
    # plt.show()

    windows_hours = list(np.multiply([8, 24], 40))
    model.predict_plot_anomalies(df_train_sanity_check, window_length=windows_hours)
    plt.suptitle("ARGUE LITE SIM Sanity check")
    plt.savefig(get_figures_path() / "ARGUELiteSim_pump30_sanitycheck_preds.png")
    # plt.show()

    # predict the test set
    model.predict_plot_anomalies(df_test, window_length=windows_hours)
    plt.suptitle("ARGUE LITE SIM Test set")
    plt.savefig(get_figures_path() / "ARGUELiteSim_pump30_testset_preds.png")
    # plt.show()
