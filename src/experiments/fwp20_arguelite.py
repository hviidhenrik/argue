...
if __name__ == "__main__":
    ...
    df_pca_test, _ = reduce_dimension_by_pca(df_test)
    plt.scatter(df_pca_data[:,0], df_pca_data[:,1], label="train", c="blue")
    plt.scatter(df_pca_test[:,0], df_pca_test[:,1], label="test", c="red")
    plt.show()
    ...
    model_path = get_serialized_models_path() / "ARGUELite_SSV_FWP20"
    if USE_SAVED_MODEL:
        ...
        model.fit(df_train.drop(columns=["partition"]),
                  epochs=None, autoencoder_epochs=50, alarm_epochs=20,
