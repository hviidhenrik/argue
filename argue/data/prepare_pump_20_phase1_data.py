...
import logging
import sys
from argue.data.ssv_fwp_data_utility_functions import *
...
if __name__ == "__main__":
    ...
    # df = pd.concat([df.loc["2018-09-01 23:59:59": "2019-09-06 23:59:59"],
    #                 df.loc["2020-02-30 23:59:59": "2020-09-14 23:59:59"]])
    df = df.loc["2018-09-01 23:59:59": "2019-09-06 23:59:59"]
    ...
    # pump 30 effect has some outliers as high as 70 - like 20 tho, it can only run at max 10 MW
    df = df.drop(
        df[
    ...
    plt.plot(df[f"effect_pump_20_MW"], 18 * df[f"effect_pump_20_MW"] + 60)
    plt.plot(df[f"effect_pump_20_MW"], 20 * df[f"effect_pump_20_MW"] + 5)

    plt.show()

    df = filter_points_beyond_linear_cut(df, x_col=f"effect_pump_{PUMP_NUMBER}_MW", y_col="flow_after_pump",
                                         slope=18, intercept=60, remove_below_cut=False)
    df = filter_points_beyond_linear_cut(df, x_col=f"effect_pump_{PUMP_NUMBER}_MW", y_col="flow_after_pump",
                                         slope=20, intercept=5, remove_below_cut=True)
    ...
    plt.plot(df_both_active[f"effect_pump_20_MW"], 18 * df_both_active[f"effect_pump_20_MW"] + 60)
    plt.show()
    df_both_active = filter_points_beyond_linear_cut(df_both_active, x_col=f"effect_pump_{PUMP_NUMBER}_MW",
                                                     y_col="flow_after_pump", slope=15, intercept=60,
    ...
    save_local_data(df_final, filename=path / "data_pump20_phase1.csv", index="timelocal")
