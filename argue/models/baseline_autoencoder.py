...
class BaselineAutoencoder(BaseModel):
    ...
    def build_model(
        ...
        self.input_to_decoders = Model(
            inputs=self.encoder.keras_model.input, outputs=decoder_output_tensor
        )
    ...
    def fit(
        ...
        autoencoder_train_dataset, autoencoder_val_dataset = train_test_split(
            x, test_size=validation_split
        )
        ...
        **kwargs,
        ...
        if window_length is not None:
            ...
            for window in window_length:
                legend_time_string = (
                    f"{window / samples_per_hour:0.0f} hour"
                    if samples_per_hour
                    else f"{window} sample"
                )
